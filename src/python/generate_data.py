...
# # Download images for these tissues
download_tissues(tissue_ID_lookup)
# use 100 of those already there
...
def main():
    ...
    for (k,t3) in enumerate(list(random_tissue_ID_lookup.keys())):
        ...
        data = []
        print (t3)
        if os.path.isfile('data/processed/patches/data_{}_{}_{}_check.py'.format(t3,tile_number,tile_level_index)):
            print ('file exists')
            continue
        ...
        pickle.dump([data,labels, IDs], open('data/processed/patches/data_{}_{}_{}_check.py'.format(t3,tile_number,tile_level_index), 'wb'))

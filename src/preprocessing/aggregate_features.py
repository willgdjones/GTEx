...
def get_expression_data(tissue):
    ...
    tissue_expression_filepath = "/nfs/research2/stegle/stegle_secure" \
        "/GTEx/download/49139/PhenoGenotypeFiles/RootStudy" \
        "ConsentSet_phs000424.GTEx.v6.p1.c1.GRU/ExpressionFiles" \
        "/phe000006.v2.GTEx_RNAseq.expression-data-matrixfmt.c1/" \
        "parse_data/44_tissues/GTEx_Data_20150112_RNAseq_RNASeQCv1.1.8" \
        "_gene_rpkm_{}_normalised_without_inverse_gene_expression.txt" \
        .format(tissue_filename)

    with open(tissue_expression_filepath, 'r') as f:
        expression_table = [x.split('\t') for x in f.read().splitlines()]
        expression_table = np.array(expression_table)
...
def get_ordered_features(IDs_group, donorIDs, aggregation):
    ...
    donorID_idx = [featuredonorIDs.index(x) for x in donorIDs]
    GTExdonorIDs = featureGTExIDs[donorID_idx]

    ordered_aggregated_features = []
#     print (GTexID_intersection)
    for gID in GTExdonorIDs:

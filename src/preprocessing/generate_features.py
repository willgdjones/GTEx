...
def main():
    for ps in patch_sizes:
        if len(glob.glob(GTEx_directory + '/data/features/{}/*{}*{}*'.format(tissue, ID, ps))) == 22:
            print("All features for ID {} patch_size {} already exist".format(ID,ps))
            continue
        ...
        patches = g['patches']
        # Runs out of 120GB RAM if use more patches
        if patches.shape[0] > 40000:
            patches = patches[0:40000,:,:,:]

        if len(patches) == 0:
            print("No patches available for ID {} patch_size {} ".format(ID, ps))
            continue
        ...
        for model_choice in model_choices:
            ...
            for l in layers:
                ...
                if len(glob.glob(GTEx_directory + '/data/features/{tissue}/{model_choice}_{ID}_{patch_size}_l{layer}_*.hdf5'.format(tissue=tissue,model_choice=model_choice,ID=ID,layer=l,patch_size=ps))) > 0:
                    ...
                    print ('Features already exist')
                    continue
                test_output = model.predict(patches[0:1])
                model_features = np.zeros([len(patches)] + list(test_output.shape[1:]),dtype=np.float16)
                ...
                for i in range(upper_limit + 1):
                    ...
                    if len(model_features.shape) == 2:
                        model_features[i*100:(i+1)*100,:] = batch_features
                    elif len(model_features.shape) == 4:
                        model_features[i*100:(i+1)*100,:,:,:] = batch_features
                    else:
                        print("Model features have wrong shape")
                        raise Exception
...
if __name__ == '__main__':
    ...
    if not all([len(glob.glob(GTEx_directory + '/data/features/{}/*{}*{}*'.format(tissue, ID, ps))) == 22 for ps in patch_sizes]):
        ...
        print("All features for ID {} already exist".format(ID))
